...
public class FlightController {
    ...
    public Flight update(@RequestBody Flight flight, @PathVariable("id") Long flightId){
        if (!flight.getFlightAvailability() && flight.getFlightBookedSeats() > 0) {
            List<String> users = flightBookingRepository.findUsersByFlight(flightId);
            eventPublisher.publishEvent(new FlightUpdateEvent(flight, users));
        }
